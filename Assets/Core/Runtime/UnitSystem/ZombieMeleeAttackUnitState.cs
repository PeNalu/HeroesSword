...
[CreateAssetMenu(menuName = "Unit States/Zombie Melee Attack State", fileName = "Zombie Melee Attack State")]
public class ZombieMeleeAttackUnitState : ScriptableUnitState
{
    ...
    protected override void ActionState(Vector3Int gridPosition, Vector3 position)
    {
        if (entityData.TryGetGridEntity(gridPosition, out GridObject gridEntity))
        {
            ...
            Team team = unitController.GetTeam();
            if(team == Team.Red)
            {
                unitController.TemporarySetTeam(Team.Blue, 1);
            }
            else
            {
                unitController.TemporarySetTeam(Team.Red, 1);
            }
